...
class mainScreen(baseui.mainScreenUI):
    ...
    def __init__(self, master=None, **kw):
        ...
        self.vfo_Button['text'] = self.VFO_A        #Specifies which VFO is active. Also
        ...
        #
        self.arrow_down_normal = gv.get_image(gv.DOWNARROWBUTTONNORMAL,self)
        self.arrow_down_pressed = gv.get_image(gv.DOWNARROWBUTTONPRESSED,self)
        self.arrow_down_disabled = gv.get_image(gv.DOWNARROWBUTTONDISABLED,self)

        self.arrow_up_normal = gv.get_image(gv.UPARROWBUTTONNORMAL,self)
        self.arrow_up_pressed = gv.get_image(gv.UPARROWBUTTONPRESSED,self)
        self.arrow_up_disabled = gv.get_image(gv.UPARROWBUTTONDISABLED,self)

        # self.vfo_VAR.set('\nVFO\n')
        # self.speaker_VAR.set("\nSPEAKER\n")

        # print(self.vfo_VAR.get(),self.speaker_VAR.get())
...
#########################################################################################
    ...
    def cj_UX_Speaker_Toggle(self, buffer):
        ...
        if (self.speaker_Button_On):
            ...
            if  gv.config.get_SDR_Switch() == "True":
                ...
                self.speaker_Button['text'] = "MUTE"

    def relaunch_SDRPanel_CB(self, event=None):
        if self.speaker_Button_On and self.speaker_Button['text'] == "SDR" and self.theSDRWindow is None:
            self.theSDRWindow = launch_sdr_popup(self)
            if self.theSDR.connect(gv.config.get_sdr_server_ip(), gv.config.get_sdr_tcp_port()):
                # print("sdr connected")
                self.theSDR.startSDR()
    ...
    def vt_UX_SET_CW_Tone(self, buffer):
        value = self.extractValue(buffer, 10, len(buffer) - 3)
        self.tone_value_Label['text'] = value
    ...
    def cv_UX_VFO_Toggle(self, buffer):
        ...
        self.vfo_Button['text'] = self.Text_To_VFO[value]          # Update Label on button

...
Infinite flywheel tuning encoder module tracking signed velocity delta step increments across an endless 360-degree rotation path.

### API Property Reference

| Property / Feature | Standard CustomTkinter | Your `sCustomTkinter` Setup |
| :--- | :--- | :--- |
| **Instantiation** | *Not Available Natively* | `sCTkDialContinuous(master)` *(Infinite Tuning Wheel Encoder)* |
...
| `_scroll_cooldown_seconds`| *Not Available Natively* | `float` throttle limiting touchpad refresh rates to stabilize fast tuning rolls. |
| `set_position_index(delta)`| *Not Available Natively* | `Method (int)` manually advances the 3D dimple coordinates via an integer step. |
...
* **Custom Accelerated Steps:** Attaching optional click callbacks allows click events to jump values by wider intervals (e.g., jumping 2 full indices per tap).
...
def my_custom_left_click():
    ...
    tuning_dial.set_position_index(-2)  # Jump 2 steps left

def my_custom_right_click():
    if tuning_dial.cget("state") == "disabled": return
    tuning_dial.set_position_index(2)   # Jump 2 steps right
...
if __name__ == "__main__":
    # Let the operating system handle the theme mode automatically natively
    root = ctk.CTk()
